...
│  ├─ 04-data/
│  ├─ 05-notes/
│  └─ 06-local-only/          # 生成源、提示词、处理记录等不公开材料
...
事实、作者结论和我们的工程推断必须明确区分。主体科研概念图必须通过项目指定的 GPT-Image CLI 使用 GPT Image 2 生成，并标注“生成式科研示意图，不是论文原图”。只有来源数据统计图、表格可视化和网页 UI 装饰可以使用代码生成的 SVG/Canvas。
...
- GPT-Image 无损原图、提示词和生成记录放在 `inbox/06-local-only/generated-source/`，公开页面只使用经过检查和优化的 WebP/AVIF 副本。
